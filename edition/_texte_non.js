...
module.exports = (message, namel) => {
  ...
  let data = JSON.parse(fs.readFileSync(`./guilds/${message.channel.guild.name}/${message.channel.guild.name}.json`))
    ...
    if (data.answers.hasOwnProperty(namel)) {message.channel.send(`Vous pouvez entrer ici différentes réponses en cas d\'échec à l\'étape, le bot renverra l'une d'elles au hasard
